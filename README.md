...
Phase 1互換の自由入力に加え、Phase 2では版管理された8問の問題セット、客観条件の限定的な自動採点、手動評価、Memo-Nexus向けJSON出力に対応しています。クラウドAI APIや結果の外部送信は行いません。
...
Phase 2結果では、問題セットID・版、問題ID・タイトル、評価観点、実行順、採点状態、自動点、手動点、完了日時を追加します。「Memo-Nexus JSON」からは`title`、`date`、`items`、`trendSummary`を持つJSONをダウンロードできます。回答中の引用符や改行はJSON文字列として正しくエスケープされます。
